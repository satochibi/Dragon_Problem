...
class DragonModel(object):
	...
	def __init__(self):
		...
		if os.path.exists(filename) and os.path.isfile(filename):
			...
			url = 'http://www.cc.kyoto-su.ac.jp/~atsushi/Programs/VisualWorks/Dragon/dragon.txt'
...
class DragonTriangle(object):
	...
	def __init__(self, vertex1, vertex2, vertex3):
		"""ドラゴンの三角形のコンストラクタ。"""
		if DEBUG: print_doc(__name__, self)
...
def main():
	...
	if TRACE: print('{0} [{1}] {2}'.format(__name__, main.__name__, main.__doc__))
...
def print_doc(namespace, object):
	"""
	この関数を呼び出したobjectのフレームオブジェクトからメソッド名を割り出して、
	そのメソッド名でobjectの属性を引いてドキュメントを獲得し、それらを名前空間と一緒に出力する。
	"""
